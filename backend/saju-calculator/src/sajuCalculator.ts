 ...
 * 진태양시 보정
 */
function applyTrueSolarTime(hour: number, minute: number, cityName: string = '서울'): { adjustedHour: number; adjustedMinute: number } {
 ...
 * 년주 계산 - 입춘(立春) 기준
 */
function calculateYearPillar(year: number, month: number, day: number): PillarResult {
 ...
 * 월주 계산
 */
function calculateMonthPillar(year: number, month: number, day: number): PillarResult {
 ...
 */
export function calculateDayPillar(year: number, month: number, day: number): PillarResult {
...
function calculateHourPillar(
  ...
  if (time >= 23 || time < 1) hourIndex = 0;
  else if (time < 3) hourIndex = 1;
  else if (time < 5) hourIndex = 2;
  else if (time < 7) hourIndex = 3;
  else if (time < 9) hourIndex = 4;
  else if (time < 11) hourIndex = 5;
  else if (time < 13) hourIndex = 6;
  else if (time < 15) hourIndex = 7;
  else if (time < 17) hourIndex = 8;
  else if (time < 19) hourIndex = 9;
  else if (time < 21) hourIndex = 10;
  else hourIndex = 11;
 ...
 * 오행 균형 계산
 */
export function calculateFiveElements(pillars: FourPillarsResult): FiveElements {
 ...
 * 일간(日干) 추출
 */
export function getDayMaster(pillars: FourPillarsResult): string {
