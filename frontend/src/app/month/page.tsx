  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // 월간 콘텐츠 로드
        const content = await api.content.getMonthly(token, currentYear, currentMonth, profile.roles[0])
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">이번 달의 흐름</h1>
              <p className="text-sm text-gray-600">{currentYear}년 {currentMonth}월</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-4">월간 콘텐츠</h2>

          {/* TODO: MonthlyContent 타입이 정의되면 구체적인 렌더링 추가 */}
          <div className="text-gray-700">
            <p className="mb-4">월간 콘텐츠 구조는 Phase 3에서 정의될 예정입니다.</p>
            <pre className="bg-gray-100 p-4 rounded overflow-auto text-sm">
              {JSON.stringify(monthlyContent, null, 2)}
            </pre>
          </div>
        </div>
      </main>
