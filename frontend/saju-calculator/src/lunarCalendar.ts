 ...
 * 음력 날짜를 포맷팅된 문자열로 반환
 */
export function formatLunarDate(date: Date, includeYear: boolean = false): string {
 ...
 * 24절기 데이터 (양력 기준 근사값)
 */
export const SOLAR_TERMS: Record<string, { month: number; day: number }> = {
  '입춘': { month: 2, day: 4 },
  '우수': { month: 2, day: 19 },
  '경칩': { month: 3, day: 6 },
  '춘분': { month: 3, day: 21 },
 ...
 * 특별한 음력 날짜인지 확인 (명절)
 */
export function getSpecialLunarDay(date: Date): string | null {
  ...
  if (lunar.month === 1 && lunar.day === 15) return '정월대보름';
  if (lunar.month === 5 && lunar.day === 5) return '단오';
  if (lunar.month === 7 && lunar.day === 7) return '칠석';
  if (lunar.month === 7 && lunar.day === 15) return '백중';
  if (lunar.month === 8 && lunar.day === 15) return '추석';
  if (lunar.month === 9 && lunar.day === 9) return '중양절';
